...
const RelatedTherapists = ({speciality, therapistId}) => {
    ...
    const [relTherap, setRelTherap] = useState([])

    useEffect(()=> {
        if(therapists.length > 0 && speciality) {
            const therapistsData = therapists.filter((therap)=> therap.speciality === speciality && therap._id != therapistId)
            setRelTherap(therapistsData)
        }
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
        ...
        <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
            {
                relTherap.slice(0,5).map((item,index)=>(
        ...
        <button onClick={()=>{navigate(`/therapists`); scrollTo(0,0 )}} className='bg-blue-50 text-gray-600 px-12 py-3 rounded-full mt-10'>View All</button>
